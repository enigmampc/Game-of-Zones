...
这篇文章介绍了三种创建`enigmagozd`节点的测试网的方式，每种针对不同的使用场景：
...
启动将会把`enigmagozd`相关的所有数据放在`~/.enigmagozd`目录。你可以检查所创建的 genesis 文件——`~/.enigmagozd/config/genesis.json`。同时`enigmagozcli`也已经配置完成并且有了一个拥有 token 的账户(stake 和自定义的代币)。
...
编译`enigmagozd`二进制文件(linux)和运行`localnet`命令所需的`tendermint/gaianode` docker images。这个二进制文件将被安装到 container 中，并且可以更新重建 image，因此您只需要构建一次 image。
...
`make localnet-start`命令通过调用`enigmagozd testnet`命令在`./build`中创建了一个 4 节点测试网络的文件。输出`./build`目录下一些文件:
...
每个`./build/nodeN`目录被挂载到对应 container 的`/enigmagozd`目录。

### 日志输出

日志被保存在每个`./build/nodeN/enigmagozd/gaia.log`文件中。你也可以直接通过 Docker 来查看日志：
...
你需要使用指定节点的`enigmagozcli`目录作为你的`home`来同`enigmagozcli`交互，并执行查询或者创建交易:
...
::: 提示
注意：每个节点的密钥种子放在`./build/nodeN/enigmagozcli/key_seed.json`中，可以通过`enigmagozcli keys add --restore`命令来回复。
:::
