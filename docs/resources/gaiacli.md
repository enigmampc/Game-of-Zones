...
`enigmagozcli` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not. Let us set it up properly. In order to install it, follow the [installation procedure](../gaia-tutorials/installation.md).

### Setting up enigmagozcli

The main command used to set up `enigmagozcli` is the following:
...
- `cosmos`
  - Derived from account keys generated by `enigmagozcli keys add`
...
- `cosmosvalconspub`
  - Generated when the node is created with `enigmagozd init`.
  - Get this value with `enigmagozd tendermint show-validator`
...
Older versions of `enigmagozcli` used store keys in the user's home directory. If you are migrating
from an old version of `enigmagozcli` you will need to migrate your old keys into your operating system's
credentials storage by running the following command:

```bash
enigmagozcli keys migrate
```
...
When broadcasting transactions, `enigmagozcli` accepts a `--broadcast-mode` flag. This
...
```bash
enigmagozcli tx send <sender_key_name_or_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id>
```
...
```bash
enigmagozcli tx send <sender_key_name_or_address> <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --dry-run
```
...
```bash
enigmagozcli tx send <sender_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id> \
...
::: tip Note
The `--generate-only` flag prevents `enigmagozcli` from accessing the local keybase.
...
where `[name]` is the name of the key you specified when you initialized `enigmagozd`.
...
```bash
enigmagozcli query distribution community-pool
```
...
```bash
enigmagozcli tx send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 1000000uatom \
