...
The genesis file is stored in `~/.enigmagozd/config/genesis.toml`.
...
This command will add all the `gentxs` stored in `~/.enigmagozd/config/gentx` to the genesis file. In order to create a genesis transaction, click [here](../validators/validator-setup.md#participate-in-genesis-as-a-validator).
