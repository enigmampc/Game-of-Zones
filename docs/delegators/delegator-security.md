...
_To protect your ATOM, you should only download software directly from official sources, and make sure that you're always using the latest, most secure version of `enigmagozcli` when you're doing anything that involves your 12 words_. The latest versions of `Tendermint`, the `Cosmos-SDK`, and `enigmagozcli` will always be available from our official Github repositories.
